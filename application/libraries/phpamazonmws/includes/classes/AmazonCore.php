  ...
  <RequestID>123</RequestID>
    ...
    protected function checkResponse($r){
        ...
        if ($r['code'] == 200){
            ...
            if (!empty($store['YourAmazonStore']['fdc_requesttype']))
            {
            	$this->log("Bad Response! made it here3",'Urgent');
            	if ($store['YourAmazonStore']['fdc_requesttype'] == "getasin")
            	{
            		$this->log("Bad Response! made it here4",'Urgent');
            		
            		if (substr_count($xml->Message,"does not have access to the given marketplace ") >0 )
            		{
            			$this->log("Bad Response! made it here5",'Urgent');
            			$tmp = explode("does not have access to the given marketplace ",$xml->Message);
						$tmpData = $tmp[1];
						$tmpData = preg_replace("/[^a-zA-Z0-9]+/", "", $tmpData);
            			
            			if (!empty($tmpData))
            			{
            				$this->log("UPDATE feedback_asin SET errored = 1 WHERE ID_ACCOUNT = '" . $store['YourAmazonStore']['ID_ACCOUNT']. "' and completed = 0 AND errored = 0 AND mws_marketplaceid = '$tmpData'",'Urgent');
            				db_query("UPDATE feedback_asin SET errored = 1 WHERE ID_ACCOUNT = '" . $store['YourAmazonStore']['ID_ACCOUNT']. "' and completed = 0 AND errored = 0 AND mws_marketplaceid = '$tmpData'",__FILE__,__LINE__,$db);
            			}
           			}
            	}
            }
            ...
            if (substr_count($xml->Code,"InvalidAccessKeyId") >0 )
            {
            	if (!empty($store['YourAmazonStore']['ID_ACCOUNT']))
            	{
            		global $db;
            		$store['YourAmazonStore']['ID_ACCOUNT'] = (int) $store['YourAmazonStore']['ID_ACCOUNT'];	
            		db_query("UPDATE feedback_settings SET apiactive = 0 WHERE ID_ACCOUNT = '" . $store['YourAmazonStore']['ID_ACCOUNT']. "'",__FILE__,__LINE__,$db);
            		
            	}
            	else
            	{
		     			if (!empty($store['YourAmazonStore']['merchantId']))
		            	{
			            	global $db;
			            	 $this->log("Bad Response! Removed: ". $store['YourAmazonStore']['merchantId'],'Urgent');
			            	db_query("UPDATE feedback_settings SET apiactive = 0 WHERE mws_sellerid = '" . $store['YourAmazonStore']['merchantId']. "'",__FILE__,__LINE__,$db);
		            	}
            		
            	}
            	
            	
            	$this->log("Bad Response! made it here: ". $store['YourAmazonStore']['ID_ACCOUNT'] . " : ",'Urgent');
