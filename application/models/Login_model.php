...
class Login_Model extends CI_Model{
    public function getUserToken($id){
        return $this->db->select('token')->where('ID',$id)->get('users')->row_array();
    }
    public function getUserDetails($id){
        return $this->db->select('active,activate_code,deleted')->where('ID',$id)->get('users')->row_array();
    }
	...
	{
		return $this->db->select("ID,email,password,token,active,ID_ACCOUNT,user_role")
        ->where('deleted','0')
	...
	public function getUserByUsername($username) 
	{
		return $this->db->select("ID,email,password,token,active")
            ->where('deleted','0')->where("username", $username)->get("users");
	}
    ...
    {
        $q=$this->db->where('deleted','0')->where("ID_ACCOUNT", $id)->get("users");
        return $q->result();
    }
}
