...
class Get_inventory_supplied extends CI_Controller {
    function __construct() {
        ...
        $this->cronName = "Get Inventory Supplied Daily";
    }

    public function index()
    {
        $content = date( "d-m-Y H:i:s" ) . " " . "Get Inventory Supplied Daily cron Start";
        $this->logging->write_log( $this->cronName, $content, 1, 0 );
        $result = $this->Inventory_shipment_model->get_inventory_shipment_feedback_setting();
    ...
    public function getinventoryshipment( $result )
    {
        ...
        foreach ( $result->result_array() as $userAccountDetailArray )
        {
            ...
            {
                echo $userAccountDetailArray['ID_ACCOUNT'];
                ...
                $endDate               = date( 'Y-m-d 23:59:59', strtotime( $startDate . ' - 4 days' ) );
                ...
                $inboundShipmentObject->setLastUpdatedAfter( str_replace( ' ', 'T', $endDate . 'Z' ) );
                $inboundShipmentObject->setLastUpdatedBefore( str_replace( ' ', 'T', $startDate . 'Z' ) );
                $responseValue         = $this->invokeListInboundShipments( $service, $inboundShipmentObject );
                $inboundShipmentDetail = array();
                    ...
                    if ( is_array( $ConvertedValue ) ) {
                        foreach ( $ConvertedValue as $finalShipmentValue ) {
                            $data                 = array(
                                ...
                                'shipment_from_date'       => $startDate,
                                'shipment_to_date'         => $endDate,
                            );
        ...
        $content = date( "d-m-Y H:i:s" ) . " " . "Get Inventory Supplied Daily cron End";
