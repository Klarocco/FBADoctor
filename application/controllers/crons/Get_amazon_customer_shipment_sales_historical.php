...
class Get_amazon_customer_shipment_sales_historical extends CI_Controller {
    function __construct() {
        ...
        $this->cronName = "Historical Customer Shipment Sales Cron";
    }
    function index() {
        $content = date("d-m-Y H:i:s") . " " . "Historical Customer Shipment Sales Cron Start";
        $this->logging->write_log($this->cronName, $content, 1, 0);
        $result = $this->Customer_shipment_sales_model->get_customer_sales_feedback_setting_historical();
        $this->getfbaorderfromamazon($result);
    }
    function getfbaorderfromamazon($result) {
        global $store, $AMAZON_SERVICE_URL;
        foreach ($result->result() as $row) {
            $amzdateiteration= $this->feedback_fba_order_model->getamzdateiterationdata('CustomersalesStatus',$row->ID_ACCOUNT);
                    ...
                    try {
                        ...
                        if (!empty($response['ReportRequestId'])) {
                            ...
                            if ($response['ReportProcessingStatus'] == '_DONE_NO_DATA_' || $response['ReportProcessingStatus'] == '_DONE_' && $row->loadedfirstorderfba = 0) {
                                $fulldataload = 1;
                                $feedStatus = 2;
                                $amzstatus = array('CustomersalesStatus' => '1');
                            ...
                            if ($fulldataload == 1) {
                                $this->feedback_setting_model->update_feedback_setting($row->ID_ACCOUNT, array(
                                    'loadfirstcustomersales' => "loadfirstcustomersales+1",
                                ));
                            }
                        }
                    } catch (Exception $ex) {
                        $feedStatus = 0;
                        $content = date("d-m-Y H:i:s") . " " . "Historical Customer Shipment Sales Cron Start" . " :RequestReportList " . $ex->getMessage();
                        $this->logging->write_log($this->cronName, $content, 1, 0);
                    }
                }
...
//                sleep(2);
                ...
                $data = array('CustomerShipmentStatus' => '1');
                $this->Customer_shipment_sales_model->historycronstatusupdate_data($data, $row->ID_ACCOUNT);
            }
        }
        $content = date("d-m-Y H:i:s") . " " . "Historical Customer Shipment Sales Cron End.";
