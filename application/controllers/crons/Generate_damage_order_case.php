...
class Generate_damage_order_case extends CI_controller
{
    ...
    {
        $cronName = "Damage Order Case";
        // Logging in database.
        $content = date("d-m-Y H:i:s") . " " . "Damage Order Case cron Start";
                ...
                if ($allRecordsFetched['FinanceRefundStatus'] == 1 && $allRecordsFetched['ReimburseStatus'] == 1 && $allRecordsFetched['OrderReturnStatus'] == 1 && $allRecordsFetched['OrderStatus'] == 1) {
                    ...
                    foreach ($allpaymentDetails as $key => $record) {
                        ...
                        if ($CurrentDate45 < $currentDate) {
                            ...
                            //$customerReturnDetails = $this->reimbursement_model->getReturnCustomerOrderDetails($record['order_id']);
                            $customerReturnDetails = $this->reimbursement_model->getReturnOrderDetails($record['order_id'], $user['ID_ACCOUNT']);
                            ...
                            if (!empty($customerReturnDetails) && empty($reimbursementDetails) && !empty($checkorderID)) {
                                if ($customerReturnDetails['detailed_disposition'] == 'DEFECTIVE' || $customerReturnDetails['detailed_disposition'] == 'CUSTOMER_DAMAGED' || $customerReturnDetails['detailed_disposition'] == 'DAMAGED') {
                                    $t = time();
                                    $data = array('case_type' => 'RDA', 'status' => '1');
                                    $effectedRow = $this->reimbursement_model->updatePaymentwithCORtype($data, $record['id']);
                                    if ($record['amount'] < 0) {
                                        $data = array('ID_ACCOUNT' => $record['ID_ACCOUNT'], 'order_id' => $record['order_id'], 'status' => '0','case_type'=>'RDA','date' => date('Y-m-d h:i:s'));
                                        $insertId = $this->reimbursement_model->insertCaseId($data);
                                        $log_data = array('ID_ACCOUNT' => $user['ID_ACCOUNT'], 'case_type' => 'RDA', 'inventory_order_id' => $record['order_id'], 'timestamp' => $t);
                                    }
        ...
        // Logging in Database.
        $content = date("d-m-Y H:i:s") . " " . "Damage Order Case cron End";
